...
mod tests {
    ...
    fn test_vedic_contract_snapshot() {
        ...
        assert!(
            !res.report.all_karakas.is_empty(),
            "all_karakas must be populated"
        );
        assert!(
            !res.report.chara_dasha_timeline.is_empty(),
            "chara_dasha_timeline must be populated"
        );

        // 2. Gochara Explainability Verification
        for transit in &res.gochara.transits {
            assert!(
                !transit.summary.is_empty(),
                "Gochara summary must not be empty"
            );
            assert!(
                !transit.description.is_empty(),
                "Gochara description must not be empty"
            );
            assert!(
                !transit.reasons.is_empty(),
                "Gochara reasons must not be empty"
            );
        }
