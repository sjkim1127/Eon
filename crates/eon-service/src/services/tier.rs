...
pub fn analyze(
    ...
    let transit_res = transit.as_ref().map(compute_transit_score).unwrap_or(ScoreResult {
        score: 50.0,
        highlights: vec![],
    });
    ...
    let detailed_components = compute_detailed_components(&saju, &vedic, transit.as_ref(), &transit_res);
    let destiny_raw_score = detailed_components.iter().map(|c| c.score * c.weight).sum::<f32>();
...
fn compute_detailed_components(
    ...
    components.push(DestinyComponent {
        ...
        score: saju_strength_score as f32,
        weight: 0.12,
        reasons: vec![format!("신강약 지수: {:.1}", saju.report.strength.strength_score)],
    });
    ...
    components.push(DestinyComponent {
        ...
        weight: 0.08,
        reasons: vec![format!("오행 흐름 효율: {:.1}%", saju.qi_topology.throughput * 100.0)],
    });

    // 3. Structure & Yongshin (Weight: 0.10)
    let structure_score = if matches!(saju.report.structure.structure, eon_saju::analysis::structure::StructureType::Follower) { 40.0 } else { 85.0 };
    components.push(DestinyComponent {
    ...
    let spirit_score = (saju.report.spirit_markers.auspicious.len() as f32 * 20.0).min(100.0) - (saju.report.spirit_markers.inauspicious.len() as f32 * 10.0).max(0.0);
    components.push(DestinyComponent {
        key: "spirit_markers".to_string(),
        label: "길흉신 분포".to_string(),
        score: spirit_score.clamp(0.0, 100.0),
        weight: 0.06,
        reasons: vec![format!("길신 {}개, 흉신 {}개", saju.report.spirit_markers.auspicious.len(), saju.report.spirit_markers.inauspicious.len())],
    });
    ...
    let yoga_score = (vedic.report.yogas.iter().filter(|y| matches!(y.quality, YogaQuality::VeryHigh)).count() as f32 * 30.0).min(100.0);
    components.push(DestinyComponent {
        key: "vedic_yogas".to_string(),
        label: "행성 조합(요가)".to_string(),
        score: yoga_score,
        weight: 0.10,
        reasons: vec![format!("최상급 요가 {}개 감지", vedic.report.yogas.iter().filter(|y| matches!(y.quality, YogaQuality::VeryHigh)).count())],
    });

    // 7. Planet Status (Avasthas) (Weight: 0.07)
    let avastha_score = (vedic.chart.avasthas.iter().filter(|a| matches!(a.baladi, BaladiAvastha::Bala | BaladiAvastha::Yuva)).count() as f32 / 7.0) * 100.0;
    components.push(DestinyComponent {
        key: "planet_status".to_string(),
        label: "행성 활성도".to_string(),
        score: avastha_score.clamp(0.0, 100.0),
        weight: 0.07,
        reasons: vec![format!("활성 상태(Bala/Yuva) 행성 {}개", vedic.chart.avasthas.iter().filter(|a| matches!(a.baladi, BaladiAvastha::Bala | BaladiAvastha::Yuva)).count())],
    });
    ...
    let risk_inv = 100.0 - (saju.vulnerability_report.as_ref().map(|v| v.total_crashes).unwrap_or(0) as f32 * 2.0).min(100.0);
    components.push(DestinyComponent {
        key: "stability".to_string(),
        label: "인생 안정성".to_string(),
        score: risk_inv,
        weight: 0.08,
        reasons: vec![format!("취약점(Crash) 지수: {}", saju.vulnerability_report.as_ref().map(|v| v.total_crashes).unwrap_or(0))],
    });
    ...
    let golden_score = saju.report.golden_time.as_ref().map(|gt| gt.average_score).unwrap_or(50.0);
    components.push(DestinyComponent {
        key: "golden_time".to_string(),
        label: "인생 골든타임".to_string(),
        score: golden_score,
        weight: 0.05,
        reasons: vec![saju.report.golden_time.as_ref().map(|gt| format!("최상의 시기: {}~{}세", gt.start_age, gt.end_age)).unwrap_or("골든타임 미정".to_string())],
    });
...
fn get_tier_from_score(score: f32) -> TierGrade {
    ...
    if s >= 97 { TierGrade { grade: "S+".to_string(), label: "천기".to_string(), desc: "하늘의 기틀이 잡힌 극귀(極貴)의 운명".to_string() } }
    else if s >= 90 { TierGrade { grade: "S".to_string(), label: "천운".to_string(), desc: "하늘이 돕고 땅이 비추는 대귀(大貴)의 운명".to_string() } }
    else if s >= 83 { TierGrade { grade: "A+".to_string(), label: "대길상".to_string(), desc: "복이 넘치며 만인이 부러워할 기세의 운명".to_string() } }
    else if s >= 75 { TierGrade { grade: "A".to_string(), label: "대길".to_string(), desc: "크게 길하며 성취가 확실한 운명".to_string() } }
    else if s >= 67 { TierGrade { grade: "B+".to_string(), label: "길상".to_string(), desc: "순한 기세 속에서 재능을 펼치는 운명".to_string() } }
    else if s >= 59 { TierGrade { grade: "B".to_string(), label: "길".to_string(), desc: "안정적이고 무난하게 번영할 운명".to_string() } }
    else if s >= 51 { TierGrade { grade: "C+".to_string(), label: "중상".to_string(), desc: "보통 이상의 저력이 있으며 노력이 빛을 발함".to_string() } }
    else if s >= 43 { TierGrade { grade: "C".to_string(), label: "중평".to_string(), desc: "굴곡이 있으나 능히 헤쳐 나갈 수 있는 운명".to_string() } }
    else if s >= 35 { TierGrade { grade: "D+".to_string(), label: "중하".to_string(), desc: "고비가 잦으나 인내로써 길을 열어야 함".to_string() } }
    else if s >= 27 { TierGrade { grade: "D".to_string(), label: "하평".to_string(), desc: "많은 주의를 요하며 신중한 처세가 필요한 시기".to_string() } }
    else if s >= 18 { TierGrade { grade: "E".to_string(), label: "하하".to_string(), desc: "크나큰 인고와 역경 뒤에 겨우 싹을 틔울 운명".to_string() } }
    else { TierGrade { grade: "F".to_string(), label: "난국".to_string(), desc: "길이 험난하니 수양과 지혜로 화를 피해야 함".to_string() } }
}

fn compute_risk_level(saju: &SajuAnalysisOutput, vedic: &VedicAnalysisOutput, transit_res: &ScoreResult) -> String {
    let mut risk_points = 0;
    if let Some(vr) = &saju.vulnerability_report {
        if vr.total_crashes >= 10 { risk_points += 1; }
        if vr.total_crashes >= 30 { risk_points += 2; }
    }
    ...
    if risk_points >= 4 { "high".to_string() }
    else if risk_points >= 2 { "medium".to_string() }
    else { "low".to_string() }
}
...
fn compute_transit_score(transit: &TransitAnalysisOutput) -> ScoreResult {
    let score = if let Some(f) = &transit.current_frame { clamp_score(f.score) } else { 50.0 };
    ScoreResult { score: clamp_score(score.round()), highlights: vec![] }
}
