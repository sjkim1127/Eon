...
pub fn analyze_connection(
    ...
    Ok(eon_human_design::connection::calculate_connection_chart(&res1.result, &res2.result))
}
pub fn analyze_transit(
    ...
    let composite_connection = eon_human_design::connection::calculate_connection_chart(
        &natal_res.result,
        &transit_chart,
    );
...
pub fn calculate_return(
    ...
    let (planet_id, target_long, approx_age) = match return_type {
        eon_human_design::transit::ReturnType::Solar => {
            let sun_data = engine.get_planet_full(dt, 0, 256).map_err(|e| ServiceError::HumanDesign(e.to_string()))?;
            let age_diff = target_year - dt.year();
            (0, sun_data.0, age_diff as f64) // SE_SUN
        }
        eon_human_design::transit::ReturnType::Saturn => {
            let saturn_data = engine.get_planet_full(dt, 6, 256).map_err(|e| ServiceError::HumanDesign(e.to_string()))?;
            (6, saturn_data.0, 29.4) // SE_SATURN
        }
        eon_human_design::transit::ReturnType::Chiron => {
            let chiron_data = engine.get_planet_full(dt, 15, 256).map_err(|e| ServiceError::HumanDesign(e.to_string()))?;
    ...
    let exact_return_time = engine.find_time_for_planet_longitude(search_start, target_long, planet_id)
        .map_err(|e| ServiceError::HumanDesign(e.to_string()))?;
