...
pub async fn init_db() -> Result<IdbDatabase, String> {
    let mut db_req = IdbDatabase::open_f64(DB_NAME, 1.0)
    ...
    db_req.set_on_upgrade_needed(Some(|evt: &IdbVersionChangeEvent| -> Result<(), JsValue> {
        if let None = evt.db().object_store_names().find(|n| n == STORE_NAME) {
            ...
            evt.db().create_object_store_with_params(STORE_NAME, &params)?;
        }
        Ok(())
    }));

    db_req.into_future().await.map_err(|e| format!("DB init failed: {:?}", e))
}
...
pub async fn save_profile(name: String, form_state: FormState) -> Result<UserProfile, String> {
    ...
    store.add_val(&js_val).map_err(|e| format!("Failed to save: {:?}", e))?;
...
pub async fn load_all_profiles() -> Result<Vec<UserProfile>, String> {
    ...
    let values = store.get_all().map_err(|e| format!("Get all error: {:?}", e))?.await
...
pub async fn delete_profile(id: &str) -> Result<(), String> {
    ...
    store.delete(&JsValue::from_str(id)).map_err(|e| format!("Delete error: {:?}", e))?;
