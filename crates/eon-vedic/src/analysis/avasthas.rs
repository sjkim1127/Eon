...
impl AvasthaEngine {
    pub fn calculate(pos: &VedicPosition, chart: &VedicChart) -> PlanetAvastha {
        ...
        let rasi = pos.rasi;
        let is_odd = !rasi.is_multiple_of(2);

        let baladi = if is_odd {
            if deg_in_sign < 6.0 {
                BaladiAvastha::Bala
            } else if deg_in_sign < 12.0 {
                BaladiAvastha::Kumara
            } else if deg_in_sign < 18.0 {
                BaladiAvastha::Yuva
            } else if deg_in_sign < 24.0 {
                BaladiAvastha::Vriddha
            } else {
                BaladiAvastha::Mrita
            }
        } else if deg_in_sign < 6.0 {
            BaladiAvastha::Mrita
        } else if deg_in_sign < 12.0 {
            BaladiAvastha::Vriddha
        } else if deg_in_sign < 18.0 {
        ...
        let jagradadi = if is_odd {
            ...
            } else if deg_in_sign < 20.0 {
                JagradadiAvastha::Swapna
            } else {
                JagradadiAvastha::Sushupti
            }
        } else if deg_in_sign < 10.0 {
            JagradadiAvastha::Sushupti
        } else if deg_in_sign < 20.0 {
            JagradadiAvastha::Swapna
        } else {
...
fn is_aspected_by(planet_house: u8, aspecting: VedicPlanet, chart: &VedicChart) -> bool {
    chart
        .aspects
        .iter()
        .any(|r| r.aspecting_planet == aspecting && r.aspected_houses.contains(&planet_house))
}
...
fn is_conjunct_with(pos: &VedicPosition, target: VedicPlanet, chart: &VedicChart) -> bool {
    chart
        .planets
        .iter()
        .any(|p| p.planet == target && p.rasi == pos.rasi)
}
...
fn is_conjunct_malefic(pos: &VedicPosition, chart: &VedicChart) -> bool {
    ...
    malefics
        .iter()
        .any(|&m| m != pos.planet && is_conjunct_with(pos, m, chart))
}
...
fn calculate_lajjitadi(
    pos: &VedicPosition,
    deeptaadi: &DeeptaadiAvastha,
    chart: &VedicChart,
) -> LajjitadiAvastha {
    ...
    let is_in_enemy_sign =
        *deeptaadi == DeeptaadiAvastha::Dukhita || *deeptaadi == DeeptaadiAvastha::Vikala;
    ...
    let is_conjunct_enemy = chart.planets.iter().any(|p| {
        ...
        let rel = RelationshipEngine::get_relationship(pos.planet, p.planet, chart);
        (rel == RelationshipType::Enemy || rel == RelationshipType::GreatEnemy)
            && p.rasi == pos.rasi
    });
    if is_in_enemy_sign
        || is_conjunct_enemy
        || is_aspected_by_enemy(pos, chart)
        || (pos.planet != VedicPlanet::Saturn && is_conjunct_with(pos, VedicPlanet::Saturn, chart))
    {
    ...
    let has_benefic_aspect = [
        VedicPlanet::Jupiter,
        VedicPlanet::Venus,
        VedicPlanet::Mercury,
    ]
    .iter()
    .any(|&b| b != pos.planet && is_aspected_by(pos.house_index, b, chart));
    ...
    let is_friend_sign =
        *deeptaadi == DeeptaadiAvastha::Mudita || *deeptaadi == DeeptaadiAvastha::Shanta;
    let is_conjunct_friend = chart.planets.iter().any(|p| {
        if p.planet == pos.planet {
            return false;
        }
        let rel = RelationshipEngine::get_relationship(pos.planet, p.planet, chart);
        (rel == RelationshipType::Friend || rel == RelationshipType::GreatFriend)
            && p.rasi == pos.rasi
    });
    let is_aspected_by_friend = chart.planets.iter().any(|p| {
        ...
        let rel = RelationshipEngine::get_relationship(pos.planet, p.planet, chart);
        (rel == RelationshipType::Friend || rel == RelationshipType::GreatFriend)
            && is_aspected_by(pos.house_index, p.planet, chart)
    });
    if is_friend_sign
        || is_conjunct_friend
        || is_aspected_by_friend
        || (pos.planet != VedicPlanet::Jupiter
            && is_conjunct_with(pos, VedicPlanet::Jupiter, chart))
    {
