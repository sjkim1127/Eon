...
fn test_verify_user_d1_chart() {
    ...
    use eon_vedic::config::{AyanamsaSystem, NodeCalculation, VedicConfig};
    ...
    let rahu = chart
        ...
        .unwrap();
    assert_eq!(
        get_nakshatra_name(rahu.nakshatra),
        "Ashwini",
        "Rahu should be in Ashwini"
    );
    assert!(
        rahu.pada >= 2 && rahu.pada <= 3,
        "Rahu should be in Pada 2-3 (Mean Node boundary tolerance)"
    );
    ...
    assert!(
        ketu_nak == "Chitra" || ketu_nak == "Swati",
        "Ketu should be in Chitra or Swati (Mean Node boundary)"
    );
