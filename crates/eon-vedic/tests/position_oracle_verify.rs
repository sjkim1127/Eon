...
fn verify_position_oracle_fixtures() {
    ...
    let cases: Vec<PositionOracleCase> =
    ...
    for case in cases {
        ...
        for (name, expected) in &case.expected.planets {
            ...
            let actual_pos = chart
                .planets
                .iter()
                .find(|p| p.planet == p_enum)
                .unwrap_or_else(|| panic!("Planet {} not found in chart", name));

            if let Some(exp_trop) = expected.tropical {
                common::assert_approx_eq(
                    actual_pos.tropical_deg % 360.0,
                    exp_trop % 360.0,
                    0.1,
        ...
        if let Some(expected_asc) = &case.expected.ascendant {
            if let Some(exp_sid) = expected_asc.sidereal {
                common::assert_approx_eq(
                    chart.ascendant.sidereal_deg % 360.0,
                    exp_sid % 360.0,
                    0.1,
