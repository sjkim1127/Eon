...
fn verify_location_oracle_snapshots() {
    ...
    let cases: Vec<LocationOracleCase> =
    ...
    for case in cases {
        ...
        let expected_utc = chrono::DateTime::parse_from_rfc3339(&case.expected.utc_time)
            .unwrap()
            .with_timezone(&Utc);

        assert_eq!(
            actual_utc, expected_utc,
            "Case {} UTC conversion mismatch",
            case.case_id
        );
        ...
        let chart = calc
            .calculate(actual_utc, case.input.lat, case.input.lon)
            .unwrap();

        // 3. Verify Ascendant (0.1 deg tolerance)
        common::assert_approx_eq(
            chart.ascendant.sidereal_deg,
            case.expected.asc_sidereal,
            0.1,
            &format!("Case {} Ascendant", case.case_id),
        );
        ...
        if let Some(exp_cusps) = &case.expected.cusps_sidereal {
            ...
            for (i, expected_cusp) in exp_cusps.iter().enumerate() {
                ...
                common::assert_approx_eq(
                    ...
                    *expected_cusp,
                    0.1,
                    &format!("Case {} House {} cusp", case.case_id, i + 1),
                );
            }
        }
        ...
        if let Some(exp_sr_str) = &case.expected.sunrise_jst {
            let exp_sr = chrono::DateTime::parse_from_rfc3339(exp_sr_str)
                .unwrap()
                .with_timezone(&tz);
