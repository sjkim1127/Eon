use eon_saju::{
    DestinyComplexity, DestinyDebugger, DestinyEntropy, DestinyFuzzer, KarmaLoadBalancer,
    LifePathReport, QiTopology,
};
...
impl DestinyAIAuditor {
    ...
    pub fn generate_context(
        ...
        let load_status = load_diagnostics
            .first()
        ...
        context.push_str(&format!(
            ...
            topology
                .nodes
                .iter()
                .map(|n| (n.element.hangul(), n.output))
                .collect::<Vec<_>>()
        ));
        ...
        context.push_str(&format!(
            "* Fuzzing Audit: Found {} vulnerabilities in static scan\n",
    ...
    pub fn build_agent_prompt(pillars: &eon_saju::FourPillars) -> Result<String, AiError> {
        ...
        prompt
            .push_str("당신은 다음 도구들을 호출하여 시스템의 상세 데이터를 조사할 수 있습니다:\n");
