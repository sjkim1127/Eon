...
pub fn get_solar_term_datetime(year: i32, term_index: u8) -> DateTime<Utc> {
    ...
    let approx_start = Utc.with_ymd_and_hms(year, 2, 1, 0, 0, 0).unwrap();
    
    engine.find_solar_term_time(approx_start, term_index)
        .unwrap_or_else(|_| {
            // 엔진 실패시 최후의 폴백 (근사값 계산)
            // 입춘(0번)은 대략 2월 4일
            // 각 절기/중기는 약 15.22일 간격
            let base_date = Utc.with_ymd_and_hms(year, 2, 4, 0, 0, 0).unwrap();
            let offset_days = (term_index as f64 * 15.218).round() as i64;
            base_date + chrono::Duration::days(offset_days)
        })
}
...
impl LunarCalendar {
    ...
    pub fn from_solar_internal(date: NaiveDate) -> Option<(i32, u32, u32, bool)> {
        let engine = eon_astro::AstroEngine::new();
        let kst_offset = chrono::Duration::hours(9);
        
        // 1. KST 당일의 끝 시점의 UTC 변환
        let dt_utc = Utc.with_ymd_and_hms(date.year(), date.month(), date.day(), 23, 59, 59).unwrap() - kst_offset;

        // 2. 현재 달 혹은 당일의 합삭(New Moon) 시각 찾기
        let nm_t = engine.find_new_moon_before(dt_utc).ok()?;
        ...
        let ws_t = engine.find_winter_solstice_before(nm_t + chrono::Duration::days(1)).ok()?;
        let ws_nm_t = engine.find_new_moon_before(ws_t + chrono::Duration::minutes(1)).ok()?;
        ...
        while (nm_t - curr_nm).num_minutes() > 4320 {
            let next_nm = engine.find_new_moon_before(curr_nm + chrono::Duration::days(45)).ok()?;
            
            // 중기(Zhong-qi) 포함 여부 확인
            let mut has_zq = false;
            for i in 1..=23 {
                if i % 2 == 1 { // 중기 인덱스 (1, 3, 5, ...)
        ...
        Some((lunar_year, month_num as u32, lunar_day, is_leap))
    }
    ...
    pub fn to_solar(year: i32, month: u32, day: u32, is_leap: bool) -> Option<NaiveDate> {
        ...
        for i in -60..360 {
            ...
            if let Some((ly, lm, ld, il)) = Self::from_solar(date) {
