...
pub fn get_solar_term_datetime(year: i32, term_index: u8) -> Result<DateTime<Utc>, crate::error::DataError> {
    // 1. 바이너리 캐시에서 먼저 조회
    ...
    let approx_start = Utc.with_ymd_and_hms(year, 2, 1, 0, 0, 0)
        .single()
    ...
    match engine.find_solar_term_time(approx_start, term_index) {
        ...
        Err(_) => {
            ...
            let base_date = Utc.with_ymd_and_hms(year, 2, 4, 0, 0, 0)
                .single()
                .ok_or(crate::error::DataError::InvalidDate)?;
...
impl LunarCalendar {
    ...
    pub fn from_solar_internal(date: NaiveDate) -> Result<(i32, u32, u32, bool), crate::error::DataError> {
        let engine = eon_astro::AstroEngine::new();
        let kst_offset = chrono::Duration::hours(9);
        
        // 1. KST 당일의 끝 시점의 UTC 변환
        let dt_utc = Utc.with_ymd_and_hms(date.year(), date.month(), date.day(), 23, 59, 59)
            .single()
            .ok_or(crate::error::DataError::InvalidDate)? - kst_offset;
    ...
    /// 음력을 양력으로 변환
    pub fn to_solar(year: i32, month: u32, day: u32, is_leap: bool) -> Result<NaiveDate, crate::error::DataError> {
