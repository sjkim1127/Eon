...
impl SignatureScanner {
    pub fn scan(frame: &LifeFrame, dynamic: &DynamicLuckAnalysis) -> Vec<LuckSignature> {
        ...
        if Self::check_double_harmony(dynamic) {
            detected.push(LuckSignature {
                ...
                description:
                    "천간과 지지가 모두 합이 되어 환경과 정신이 완벽한 조화를 이루는 시기입니다."
                        .to_string(),
            });
        }
        ...
        if frame.score < 30.0 && frame.tags.iter().any(|t| t.contains_pattern("충")) {
            detected.push(LuckSignature {
                ...
                description:
                    "핵심 에너지가 강력한 충돌로 인해 기반이 크게 흔들리는 위기 상황입니다."
                        .to_string(),
            });
        }

        // 3. 재공귀인 (Gong-Gwi - 가상 패턴: 복합적인 합의 중첩)
        let branch_hap_count = dynamic
            .combined_relations
            .six_combinations
            .iter()
            .filter(|(_, p1, p2)| p1.contains("운") || p2.contains("운"))
            .count()
            + dynamic
                .combined_relations
        ...
        if branch_hap_count >= 2 && frame.score > 65.0 {
            detected.push(LuckSignature {
                ...
                severity: SignatureSeverity::Success,
                description: "보이지 않는 기운이 명예와 재물을 끌어오는 강력한 성취의 시기입니다."
                    .to_string(),
            });
        }
        ...
        if frame.score > 85.0 && frame.tags.iter().any(|t| t.contains_pattern("핵심운")) {
            detected.push(LuckSignature {
                ...
                severity: SignatureSeverity::Success,
                description:
                    "인생의 큰 관문을 넘어 신분이 상승하거나 큰 조직의 수장이 되는 운세입니다."
                        .to_string(),
            });
        }
    ...
    fn check_double_harmony(dynamic: &DynamicLuckAnalysis) -> bool {
        // 대운이나 세운이 개입된 합만 필터링
        let has_dynamic_stem_hap = dynamic
            .combined_relations
            .stem_combinations
            .iter()
            .any(|(_, p1, p2)| p1.contains("운") || p2.contains("운"));

        let has_dynamic_branch_hap = dynamic
            .combined_relations
            .six_combinations
            .iter()
            .any(|(_, p1, p2)| p1.contains("운") || p2.contains("운"))
