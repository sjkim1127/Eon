...
use crate::relationships::{RelationshipAnalysis, TripleCombination, SeasonalCombination};
...
impl DynamicLuckAnalysis {
    ...
    pub fn analyze(
        ...
        let major_influence = major.map(|m| {
            Self::get_influence(m, "대운", natal)
        });
        
        let yearly_influence = yearly.map(|y| {
            Self::get_influence(y, "세운", natal)
        });
    ...
    fn analyze_expanded(
        ...
        // 모든 쌍에 대한 분석 로직 호출 (생략 없이 구현 필요)
        use crate::relationships::{StemCombination, StemClash, SemiCombination, SixCombination, BranchClash, BranchPunishment, BranchHarm, Amhap, MyungAmHap};
