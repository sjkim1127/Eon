...
if [[ -z "$RAW_WASM" ]]; then
  echo "Dioxus did not create a raw WebAssembly executable." >&2
...
if [[ -z "$WASM_BINDGEN" ]]; then
  echo "Dioxus wasm-bindgen executable was not found under $BINDGEN_DIR." >&2
...
if [[ ! -s "$JS_OUTPUT" || ! -s "$WASM_OUTPUT" ]]; then
  echo "wasm-bindgen completed without producing the expected bundle." >&2
  find "$WASM_DIR" -maxdepth 3 -printf '%y %p %s bytes\n' 2>/dev/null | sort >&2 || true
  exit 1
fi

echo "Generated WebAssembly bundle:"
