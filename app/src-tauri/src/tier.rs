...
pub fn get_destiny_tier_analysis(
    ...
    let saju: SajuInputResult = serde_json::from_value(saju_val).map_err(|e| format!("Saju Parse Err: {}", e))?;
    let vedic: VedicInputResult = serde_json::from_value(vedic_val).map_err(|e| format!("Vedic Parse Err: {}", e))?;
    let transit: TransitInputResult = serde_json::from_value(transit_val).map_err(|e| format!("Transit Parse Err: {}", e))?;
